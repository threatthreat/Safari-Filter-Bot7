...
async def pm_text(bot, message):
    ...
    if await db.get_setting("PM_FILTER", default=PM_FILTER) or await db.has_premium_access(message.from_user.id):
        ...
        await message.reply_text("<b>ɪғ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴛᴀᴋᴇ ᴀ ᴍᴏᴠɪᴇ ғʀᴏᴍ ᴛʜᴇ ʙᴏᴛ, ʏᴏᴜ ᴍᴜsᴛ ᴘᴀʏ ғᴏʀ ᴘʀᴇᴍɪᴜᴍ ᴏᴛʜᴇʀᴡɪsᴇ ʏᴏᴜ ᴄᴀɴ ɢᴇᴛ ᴛʜᴇ ᴍᴏᴠɪᴇ ғʀᴏᴍ ᴛʜᴇ ɢʀᴏᴜᴘ /n/n (💸 40 Rs / ᴍᴏɴᴛʜ ⏱️)</b>", reply_markup=InlineKeyboardMarkup([
    ...
    try:
        ...
        if not spoll:
            ...
            if len(message.text) < 100:
                search = message.text
                m=await message.reply_sticker(sticker="CAACAgUAAxkBAAKsH2gPku-SXoMZwT-dTAsCPreQ-O45AAI6DwAC0Bh5Vs_uiKOtnAfnNgQ",
